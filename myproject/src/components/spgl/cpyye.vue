<template>
  <div class="wrapper">
    ...
    <el-card>
      ...
      <div id="myEcharts2" style="width:100%;min-height: 300px;margin-top: 50px"/>
      <!-- 分页 -->
      <el-pagination
...
export default {
  ...
  methods: {
    fetchData(params) {
      this.$http
          .post("http://localhost:3000/bxpc/cpyye", params)
          .then((response) => {
            if (response.data.code === 200) {
              this.listData = response.data.data;
              this.total = response.data.total;
              createEcharts("myEcharts2", "产品营业额", this.listData.map(val => ({
